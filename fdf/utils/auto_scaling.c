...
/*   Updated: 2022/02/22 17:22:05 by lduboulo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	scaling_adjustment(t_fdf *fdf)
{
			...
			|| (x_min > 250 || x_min < 75))
	{
		...
		x_max = fdf->res.x0 + fdf->map.nb * fabs(fdf->res.x_scale * cos(ALPHA));
		x_min = fdf->res.x0 - (fdf->txt.nbline - 1) * fabs(fdf->res.x_scale * cos(ALPHA));
	}
	y_max = 100;
...
void	scaling_adjustment_y_max(t_fdf *fdf)
{
	...
	y_max = fdf->res.y0 + (fdf->map.nb + fdf->txt.nbline) * fabs(fdf->res.y_scale * sin(ALPHA));
